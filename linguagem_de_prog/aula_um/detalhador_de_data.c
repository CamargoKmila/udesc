...
int main() {
    ...
    const int ano_minimo = 1800;
    const int ano_maximo = 2199;
    
    printf("\tBEM VINDO AO DETALHADOR DE DATA %i!!!\n\n", ano_atual);
    printf("Informe um dia (entre 1 e 31)\n");
    scanf("%i", &dia);
    if (dia < 1 || dia > 31) {
      printf("Informe um dia valido!\a");
      return 0;
    }
    ...
    scanf("%i", &mes);
    if (mes < 1 || mes > 12) {
      printf("Informe um mes valido!\a");
      return 0;
    }
    ...
    if (ano < ano_minimo || ano > ano_maximo) {
      printf("Informe um ano valido!\a");
      return 0;
    }
